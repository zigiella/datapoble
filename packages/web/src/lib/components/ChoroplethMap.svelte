<script lang="ts">
	/**
	 * Mapa de Catalunya (MapLibre GL · Mirador) — APARADOR OFICIAL (fase nova, model aparcat).
	 *
	 ...
	 *  - TOT CATALUNYA visible (els 947 municipis), pintat per l'indicador OFICIAL actiu: el
	 *    Berguedà des del dataset ric i la resta des de `catValues` (escala CAT), amb la MATEIXA
	 *    classificació → colors comparables a tot el país;
	 *  - «sense dada» de l'indicador → TRAMAT (hatch) al Berguedà i fill atenuat a la resta, mai
	 *    relleno pla (secret estadístic, honestedat);
	 *  - LÍMITS DE COMARCA suaus a sobre (de catalunya-comarques.geojson) per orientar;
	 *  - els 31 del Berguedà amb contorn una mica més marcat per distingir-los del context;
	 *  - hover/selecció a tot el país (el component emet `inBergueda` perquè la pàgina triï tooltip).
	 *
	 * MODEL APARCAT (vot de Bea 2026-07-16): fora la capa de cobertura del Nivell C, la banda de
	 * pernocta del tooltip i el tractament de confiança del model (vel + trama): la confiança era
	 * un diagnòstic DEL MODEL i no ha de velar dades oficials.
	 *
	...
	interface HoverPayload {
		...
		/** True si el municipi és del Berguedà (dataset ric); fals → dada de catValues o cap. */
		inBergueda: boolean;
		x: number;
		y: number;
	}
	...
	interface Props {
		...
		/** Valors d'indicadors OFICIALS a escala Catalunya (ine5 → {clau → valor}) per pintar
		 * tots els municipis pel mateix indicador que el Berguedà. Opcional. */
		catValues?: IndicadorsCatData;
		/** Enquadrament inicial: 'bergueda' (defecte), 'catalunya' (tot el país) o una llista
		 *  d'ine5 (p. ex. els municipis d'una comarca). */
	...
	const COVERED = 'mun-covered'; // munis de fora del Berguedà amb dada oficial (catValues)

	let mlgl: typeof import('maplibre-gl') | null = null; // ref per re-enquadrar

	/**
	 * Conjunt de codis INE5 amb dades riques = els municipis del Berguedà (les claus del dataset
	 * són exactament els 31). Es deriva del dataset perquè la frontera «té dades / no en té»
	 * segueixi la font de dades, no una llista codificada.
	 */
	const bergSet = $derived(new Set(Object.keys(dataset.municipis)));
	// Munis FORA del Berguedà amb dada oficial per pintar (catValues, escala Catalunya). El pintat
	// real per muni el decideix `__covval` (el valor de l'indicador ACTIU): si el muni no en té,
	// queda transparent → atenuat (honest).
	const coveredSet = $derived(
		new Set(Object.keys(catValues ?? {}).filter((i) => !bergSet.has(i)))
	);
	 ...
	 * (__val/__hasval); si és de la resta de Catalunya amb dada oficial (catValues), el valor
	 * per a la capa COVERED (__covval). Sense dada → capa base atenuada.
	 * (El tractament de confiança del model —__conf/__lowconf— està aparcat amb el model.)
	 */
	function joinValues(fc: FeatureCollection, key: MetricKey): FeatureCollection {
		...
		return {
			...
			features: fc.features.map((f) => {
				...
				const covered = !inBerg && cov.has(ine5); // dada oficial a escala CAT (catValues)
				// Valor de l'INDICADOR ACTIU per al muni cobert: null si no en té → atenuat (honest).
				...
				if (categorical) {
					...
					// conegut. Si el valor falta o no és reconegut → tramat.
	...
	async function init() {
		...
		map.on('load', () => {
			...
			// Capa COBERTS: municipis FORA del Berguedà amb dada OFICIAL a escala Catalunya
			// (catValues). Es pinten pel MATEIX indicador actiu que el Berguedà (mateixa
			// classificació i colors); si el muni no té valor, opacitat 0 (queda atenuat sobre la
			// capa BASE, honest «sense dada»).
			map.addLayer({
				id: COVERED,
				type: 'fill',
				source: SRC,
			...
			// Capa coroplètica: municipis del Berguedà amb dada de l'indicador actiu.
			map.addLayer({
				id: FILL,
				type: 'fill',
				source: SRC,
	...
	/** Enquadra a la bbox dels municipis del BERGUEDÀ (els que tenen dades riques). */
	function fitToBergueda() {
		fitToFeatures(geojson, (f) => bergSet.has((f.properties?.ine5 as string) ?? ''));
	}

	/** Enquadrament segons `fitTo`: Catalunya sencera, una llista d'ine5, o el Berguedà. */
	function applyFit() {
		if (!map) return;
		if (fitTo === 'catalunya') {
	...
	function wireInteractions() {
		...
		const interactive = [FILL, HATCH, BASE];
		...
		const buildHover = (feat: MapGeoJSONFeature, point: { x: number; y: number }) => {
			...
			// Valor de l'indicador actiu: del Berguedà (dataset) o, per als coberts, de catValues
			// (escala CAT) → tooltip uniforme amb el mateix indicador oficial a tot el país.
