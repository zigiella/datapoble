...
# Escala Catalunya (F5): envolupant de tot el país + geometria 947; baixada per MOSAIC de tiles.
...
def fetch_pois(bbox: tuple = BBOX, tiles: int = 1, session: requests.Session | None = None) -> list[dict]:
    """Baixa els POIs de comerç/serveis del bbox. Si `tiles>1`, per MOSAIC, deduplicant per (type,id)."""
    sess = session or requests.Session()
    ...
    for k, box in enumerate(boxes):
        ...
        for el in els:
            ...
            if key not in seen:
                ...
                elements.append(el)
        if tiles > 1:
            print(f"  [serveis] tile {k + 1}/{len(boxes)}: {len(els)} POIs (acum {len(elements)})",
                  file=sys.stderr)
    return elements
...
def assign_to_municipis(points: list[dict], geojson=GEOJSON) -> pd.DataFrame:
    ...
    desglossament per categoria. Els POIs fora dels polígons es **descarten** (retall a la frontera).
    """
    con = duckdb.connect()
...
def run(municipis: dict[str, str] = BERGUEDA, geojson=GEOJSON, bbox: tuple = BBOX, tiles: int = 1) -> dict:
    """Ingesta OSM de comerç i serveis essencials per municipi. Idempotent. `geojson`+`bbox`+`tiles`
    parametritzen l'abast: pilot Berguedà (defecte) o tot Catalunya (geometria 947 + bbox CAT + mosaic).

    Determinista quant a query i assignació; l'snapshot d'OSM pot variar amb el temps (mínim, no cens).
    """
