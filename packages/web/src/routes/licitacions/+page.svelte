<script lang="ts">
	...
	// Import en euros, forma curta i humana (42,4 M€ · 886 k€ · 1.200 €).
	function eur(v: number | null): string {
		if (v == null) return '—';
		if (v >= 1e6) return `${(v / 1e6).toLocaleString(intl, { minimumFractionDigits: 1, maximumFractionDigits: 1 })} M€`;
	...
	// Etiquetes de tema (categòric nou, no al contracte → copy i18n). Fallback: la clau.
	...
	const temaLabel = (t: string) => TEMA[t]?.() ?? t;
	...
	const lecturaLabel = (l: string) => LECTURA[l]?.() ?? l;
