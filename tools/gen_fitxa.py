...
BL_ES = re.compile(r"\b(porque|debido a|a causa de|provoca|causa que|el más|la más|el mejor|el peor|sin duda|clarísimo)\b", re.I)
BL_CA = re.compile(r"\b(perquè|a causa de|degut a|provoca|causa que|el més|la més|el millor|el pitjor|sens dubte|claríssim)\b", re.I)
...
def verify(content: str, facts: dict, bl: re.Pattern) -> dict:
    res = {"valid_json": False, "missing": [], "unmatched": [], "blacklist": [], "contra_ok": True}
    ...
    res["blacklist"] = sorted({m.group(0).lower() for m in bl.finditer(content)})
    return res
...
def main(argv=None) -> int:
    ...
    for ine5 in (args.muni or MUNIS):
        ...
        print(f"  ESCRIPTOR opus-4.8 (es):  json={ves['valid_json']} falten={ves['missing']} "
              f"xifres_no_verif={len(ves['unmatched'])} negra={ves['blacklist']} contra_ok={ves['contra_ok']} {es_dt:.0f}s")
